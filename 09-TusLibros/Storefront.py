...
class Storefront:
    ...
    def __init__(self, publishers_catalog, login_system, price_list, merchant_processor, clock, time_until_expiration): #revisar nombres
        ...
        self._carts_last_time_used = {}

    def create_cart_for(self, client_id, client_password):
        self._login_system.log_in_user(client_id, client_password) #cambiar el nombre de esto y/o del mock

        cart = ShoppingCart(self._book_catalog)
        cart_id = str(self._number_of_carts)
        self._carts_last_time_used[cart_id] = self._clock.now()
    ...
    def list_cart_content(self, cart_id):
        ...
        self._check_if_expired(cart_id)
    ...
    def _check_if_expired(self, cart_id): #CAMBIAR NOMBRE 2 metodos iguales tc...
        if self._clock.now() - self._carts_last_time_used[cart_id] > self._time_until_expiration:
    ...
    def list_purchases(self, client_id, password):
        self._login_system.log_in_user(client_id, password)
